       ...
       Chart.view(actor[p].name).then(function(v){
         ...
         if(actor['grouped_bar'].view && actor['map'].view ){
           ...
           .on('click', function(e, item){
             ...
             if(!written){
               tmp = item['datum']['data']['category'];
               console.log("tmp = " + tmp);
               // a
               if(tmp <= $scope.filters['count'].range.a){
                 console.log("tmp <= range.a");
                 a = tmp;
                 last = tmp;
                 b = $scope.filters['count'].range.b;
               }else{
                 // b
                 if(tmp >= $scope.filters['count'].range.b){
                   console.log("tmp >= range.b");
                   b = tmp;
                   last = tmp;
                   a = $scope.filters['count'].range.a;
                 }else{
                   console.log("range.a < tmp < range.b");
                   if((!last) || (a === last)){
                     console.log("!last or a === last");
                     a = $scope.filters['count'].range.a;
                     b = tmp;
                   }else{
                     console.log("b === last");
