...
angular.module('app').controller('FootprintController', function($scope, $state, Mixin, Chart, Countries, Mediator, Audio) {
   ...
   var render = function(_sD){
     ...
     for(p in sD.model){
       ...
       if(actor[p]['update'] === 'false'){ continue; }
       ...
       if(p === 'textboxes'){
         actor[p]['data']['values'].forEach(function(v,i){
           for(q in v){
             label[i] = q;
             value[i] = v[q];
...
//           set actor='map' update to false - no need for update of map
//           console.log("before set f: sD.model['map'].update = " + sD.model['map'].update); 
//           sD.model['map'].update = 'false';
//           console.log("after set f: sD.model['map'].update = " + sD.model['map'].update); 
//
...
//           });
           ...
           console.log("actor['barchart'].view.update()");
           actor['barchart'].view.update({items: 'enter', duration: 1000});
